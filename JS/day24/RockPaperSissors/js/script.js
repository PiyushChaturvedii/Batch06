...
const gameReady = () => {
  ...
  const evaluateResult = (player, comp) => {
    //set the evalution after 200 mils
    setTimeout(() => {
      // if player is rock
      if (player === "rock")
        comp === "scissors" ? win() : comp === "paper" ? lose() : tie();
      // if player is paper
      if (player === "paper")
        comp === "rock" ? win() : comp === "scissors" ? lose() : tie();
      // if player is scissors
      if (player === "scissors")
        comp === "paper" ? win() : comp === "rock" ? lose() : tie();
    }, 600);
  };
